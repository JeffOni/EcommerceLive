<x-admin-layout :breadcrumbs="[
    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 relative overflow-hidden">
        ...
        <div class="text-center mb-8 pt-8">
            ...
            <p class="text-gray-600 text-lg">Modifica la información de la categoría "{{ $category->name }}"</p>
        </div>
                ...
                <div class="relative p-8">
                    <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="space-y-8">
                        ...
                        <div class="flex items-center space-x-4 mb-8">
                            <div
                                class="w-12 h-12 bg-gradient-to-r from-teal-500 to-cyan-600 rounded-xl flex items-center justify-center shadow-lg">
                                <i class="fas fa-edit text-white text-xl"></i>
                            </div>
                            <div>
                                <h2 class="text-2xl font-bold text-gray-800">Información de la Categoría</h2>
                                <p class="text-gray-600">Actualiza los datos de la categoría</p>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end gap-6 pt-6 border-t border-gray-200">
                            <button type="button" onclick="confirmDelete()"
                                ...
                                <span class="text-white">Eliminar {{ $category->name }}</span>
                            </button>
                            <button type="submit"
                                class="px-8 py-3 rounded-xl shadow-lg hover:shadow-xl bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 text-white font-semibold transition-all duration-300 transform hover:scale-105 text-lg flex items-center space-x-2">
                                <i class="fas fa-save mr-2 text-white"></i>
                                <span class="text-white">Actualizar Categoría</span>
                            </button>
                        </div>
                    </form>
                </div>
    ...
    {{-- js de sweet alert para confirmar la eliminacion --}}
    @push('js')
        <script>
            let confirmDelete = () => {
                // Sweet Alert 2
